...
//        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(origin.x, origin.y+height, dropDownViewSize.width, dropDownViewSize.height)];
        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, dropDownViewSize.width, dropDownViewSize.height)];
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    // 点击菜单进行回调
    if (self.willShowBackgroundViewBlock) {
        self.willShowBackgroundViewBlock();
    }
...
- (void)animateBackGroundView:(UIView *)view show:(BOOL)show complete:(void(^)(void))complete {
    if (show) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            // 消失时进行的操作
            if (self.willEndBackgroundViewBlock) {
                self.willEndBackgroundViewBlock();
            }
    ...
    if (tableView == _leftTableView) {
        ...
        if (_dataSourceFlags.accessoryViewForRowAtIndexPath) {
            ...
            if (_dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:indexPath.row column:_currentSelectedMenudIndex]> 0){
                ...
                cell.accessoryView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@""] highlightedImage:[UIImage imageNamed:@"waimai_select"]];
            }
        }
