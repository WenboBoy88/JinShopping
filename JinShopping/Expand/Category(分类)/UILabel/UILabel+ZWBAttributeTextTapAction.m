//
//  UILabel+ZWBAttributeTextTapAction.m
//
//  Created by 张文博 on 16/7/1.
//  Copyright © 2016年 张文博. All rights reserved.
//

#import "UILabel+ZWBAttributeTextTapAction.h"
#import <objc/runtime.h>
#import <CoreText/CoreText.h>
#import <Foundation/Foundation.h>

@implementation ZWBAttributeModel

@end

@implementation UILabel (ZWBAttributeTextTapAction)
...
- (id<ZWBAttributeTapActionDelegate>)delegate
{
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)setDelegate:(id<ZWBAttributeTapActionDelegate>)delegate
{
    objc_setAssociatedObject(self, @selector(delegate), delegate, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

#pragma mark - mainFunction
- (void)zwb_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings tapClicked:(void (^) (NSString *string , NSRange range , NSInteger index))tapClick
{
    [self zwb_getRangesWithStrings:strings];
...
- (void)zwb_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings
                                   delegate:(id <ZWBAttributeTapActionDelegate> )delegate
{
    [self zwb_getRangesWithStrings:strings];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self zwb_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        
        if (weakSelf.tapBlock) {
            weakSelf.tapBlock (string , range , index);
        }
        
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(zwb_attributeTapReturnString:range:index:)]) {
            [weakSelf.delegate zwb_attributeTapReturnString:string range:range index:index];
        }
        
        if (self.isTapEffect) {
            
            [self zwb_saveEffectDicWithRange:range];
            
            [self zwb_tapEffectWithStatus:YES];
        }
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    ...
    if (self.isTapAction) {
        if ([self zwb_getTapFrameWithTouchPoint:point result:nil]) {
            return self;
        }
    }
...
- (BOOL)zwb_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock
{
    ...
    CGAffineTransform transform = [self zwb_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self zwb_getLineBounds:line point:linePoint];
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                ZWBAttributeModel *model = self.attributeStrings[j];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    if (self.isTapEffect) {
        ...
        [self performSelectorOnMainThread:@selector(zwb_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
...
- (CGAffineTransform)zwb_transformForCoreText
{
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect)zwb_getLineBounds:(CTLineRef)line point:(CGPoint)point
{
    CGFloat ascent = 0.0f;
...
- (void)zwb_tapEffectWithStatus:(BOOL)status
{
    if (self.isTapEffect) {
...
- (void)zwb_saveEffectDicWithRange:(NSRange)range
{
...
- (void)zwb_getRangesWithStrings:(NSArray <NSString *>  *)strings
{
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf zwb_getStringWithRange:range]];
            
            ZWBAttributeModel *model = [ZWBAttributeModel new];
...
- (NSString *)zwb_getStringWithRange:(NSRange)range
{
